"""Collect answers for Ragas-custom questions using LightRAG mix mode (non-agentic).

Retrieves context chunks with mode="mix" (vector + knowledge graph), then calls
the LLM with a simple, consistent RAG prompt — no query decomposition or rewriting.

Usage:
  python -m tests.eval.collect_answers_mix
  python -m tests.eval.collect_answers_mix --limit 5
  python -m tests.eval.collect_answers_mix \\
      --questions tests/eval/data/ragas_custom_questions.json \\
      --output    tests/eval/results/answers_mix.json \\
      --top-k     5 \\
      --delay     0.5
"""
...
_DEFAULT_OUTPUT = str(RESULTS_DIR / "answers_mix.json")
...
def main(
    ...
    mode_label = "mix (official GraphRAG-Bench protocol)" if official else "mix"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Collect LightRAG mix answers")
